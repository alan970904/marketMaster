...
public class CheckoutDetailsDao {
    ...
    public CheckoutDetailsBean getOne(Session session, String checkoutId, String productId) {
        try {
            return session.get(CheckoutDetailsBean.class, new CheckoutDetailsBean.CheckoutDetailsId(checkoutId, productId));
        } catch (Exception e) {
    ...
    public List<CheckoutDetailsBean> getPart(Session session, String checkoutId) {
        try {
            Query<CheckoutDetailsBean> query = session.createQuery("FROM CheckoutDetailsBean cd WHERE cd.checkoutId = :checkoutId", CheckoutDetailsBean.class);
    ...
    public List<CheckoutDetailsBean> searchByProductId(Session session, String productId) {
        try {
            Query<CheckoutDetailsBean> query = session.createQuery("FROM CheckoutDetailsBean cd WHERE cd.productId LIKE :productId", CheckoutDetailsBean.class);
    ...
    public List<Map<String, Object>> getProductReturnRates(Session session) {
        try {
            String hql = "SELECT cd.productId, CAST(ROUND((SUM(CASE WHEN rd.returnId IS NOT NULL THEN rd.numberOfReturn ELSE 0 END) * 100.0) / SUM(cd.numberOfCheckout), 2) AS java.math.BigDecimal) AS returnRate " +
                         "FROM CheckoutDetailsBean cd LEFT JOIN ReturnDetailsBean rd ON cd.productId = rd.productId AND cd.checkoutId = rd.checkoutId " +
